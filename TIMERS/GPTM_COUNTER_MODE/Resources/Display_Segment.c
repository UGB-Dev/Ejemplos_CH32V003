...
void Display_Segment_Init(void){
    ...
    /* CONFIGURACION DEL PIN PD2, PD4 Y PD5 */
    GPIOD -> CFGLR &= ~( GPIO_CFGLR_CNF2 | GPIO_CFGLR_MODE2 | 
                         GPIO_CFGLR_CNF4 | GPIO_CFGLR_MODE4 |
                         GPIO_CFGLR_CNF5 | GPIO_CFGLR_MODE5 ); // Borra configuraciones iniciales

    GPIOD -> CFGLR |=  GPIO_CFGLR_MODE2 | GPIO_CFGLR_MODE4 | //Pin PD2 y PD4 como salida a 30 MHz en Push-Pull
                       GPIO_CFGLR_CNF5_1; //  PD5 como entrada Pull-Down
...
void Imprimir(uint8_t Cont){
    for (uint8_t i=0; i< 8; i++ ) { //delay 500 ms por cuenta
        ...
        GPIOD -> BSHR = GPIO_BSHR_BS2 | GPIO_BSHR_BR4;
        Delay_Ms(4); 

        GPIOC -> OUTDR = Display[Cont%10] | GPIO_OUTDR_ODR7; // UNIDADES
        GPIOD -> BSHR = GPIO_BSHR_BS4 | GPIO_BSHR_BR2;
        Delay_Ms(4);
    }  
}