 ...
 * Version            : V1.0.0
 * Date               : 2023/12/25
 * Description        : GPTM; Se configuran los registros necesarios para que el Timer 2
...
int main(void){
    ...
    while(1){
        ...
        if (GPIOD ->INDR & GPIO_INDR_IDR5) {
            while (GPIOD ->INDR & GPIO_INDR_IDR5); // Antirrebote
            TIM2 -> CTLR1 ^= TIM_DIR; // Contador UP - DOWN 
        }

        CONTEO = Aux_Conteo = TIM2 -> CNT ; // Se captura el valor del registro CNT
