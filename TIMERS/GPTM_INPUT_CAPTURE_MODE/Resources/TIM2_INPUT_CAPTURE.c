/*
    ...
    TIM2_INPUT_CAPTURE.c

*/

#include "TIM2_INPUT_CAPTURE.h"
...
*/ 
    ...
    /* CONFIGURACION DEL TIMER2 CANAL 2 PARA EL PIN PD3*/ 
    ...
    TIM2 -> ATRLR = 100; // Cuenta maxima a la cual llegara CNT
...
/*
    ...
    /* CONFIGURACION DEL TIMER2 CANAL 2 PARA EL PIN PD3 Y PD4
    TIM2 -> CHCTLR1 &= ~( TIM_CC2S  | TIM_CC1S); // Borra configuraciones por defecto
    TIM2 -> CHCTLR1 |= TIM_CC2S_0  | TIM_CC1S_0; // Se cambia de CC (Compare Capture) a IC (Input Capture) y se mapea a TI2 y TI1
    ...
    TIM2 -> SMCFGR |=  TIM_SMS_0 | TIM_SMS_1  ; // Encoder mode 3; 
...
/*
    ...
    /* CONFIGURACION DEL TIMER2 CANAL 2 
    TIM2 -> CHCTLR1 |= TIM_CC2S_0 ; // Se cambia de CC (Compare Capture) a IC (Input Capture) y se mapea a TI2
    TIM2 -> CHCTLR1 &= ~TIM_IC2F; // Sin filtro digital
    TIM2 -> CCER &= ~TIM_CC2P; // Deteccion por flanco de subida
    TIM2 -> SMCFGR |= TIM_TS_TI2FP2 | TIM_SMS_1 | TIM_SMS_2; // Trigger mode en TI2FP2(TI2) 
    TIM2 -> CTLR1 |= TIM_OPM; // Activacion Single Mode Pulse
...
/*
    ...
    /* CONFIGURACION DEL TIMER2 CANAL 1 
    TIM2 -> CTLR1 &= ~TIM_CKD_DIV4; // Sin divisor
    TIM2 -> PSC = 24000-1; // Base de tiempo a 1ms (tick del contador)
    TIM2 -> CH1CVR = 2000; // Tiempo del delay 250 ms
    TIM2 -> ATRLR = 7000; // (CHV1CRV + 5000) * 1 ms = 7s  ; pulso de 5 segundos
    TIM2 -> CHCTLR1 |=  TIM_OC1M_2 | TIM_OC1M_1 ; // PWM en modo 1 
    TIM2 -> CCER |= TIM_CC1P | TIM_CC1E; // Se activa la salida en OC2(TIM2CH2) a nivel 0 logico  
    ...
    /* ACTIVACION DE LAS SALIDAS DEL CANAL 1 
    TIM2 -> BDTR |= TIM_MOE; // Se habilita la salida principal OC1(TIM2CH1)
    TIM2 -> CTLR1 |= TIM_CEN; // Se habilita el contador CNT
##########################################################################################*/

/*########################################################################################
    SMS; Input Compare en Modo External Clock(Modo Contador)
